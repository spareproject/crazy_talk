...
read -p "enter root pin: " pin
hexdump -C <<< $(dd if=${random} bs=1 count=100 ibs=1 skip=${pin} 2>/dev/null)
gpg --homedir ./mount/root --passphrase-fd 0 --batch --gen-key ./airootfs/etc/gnupg/batch.root <<< $(dd if=${random} bs=1 count=100 ibs=1 skip=${pin} 2>/dev/null)
gpg --homedir ./mount/root --output ./mount/root.public --export root

read -p "enter user pin: " pin
hexdump -C <<< $(dd if=${random} bs=1 count=100 ibs=1 skip=${pin} 2>/dev/null)
gpg --homedir ./mount/user --passphrase-fd 0 --batch --gen-key ./airootfs/etc/gnupg/batch.user <<< $(dd if=${random} bs=1 count=100 ibs=1 skip=${pin} 2>/dev/null)
gpg --homedir ./mount/user --output ./mount/user.public --export user
gpg --homedir ./mount/user --import ./mount/root.public
gpg --homedir ./mount/user --passphrase-fd 0 --sign-key root <<< $(dd if=${random} bs=1 count=100 ibs=1 skip=${pin} 2>/dev/null)
